...
class Board:
    ...
    color={
     ...
     # constructor: It opens the game window and initializes the flag variables
    def __init__(self):
        self.n=4
        self.window=Tk()
    ...
    #If the value is zero, it displays no text and gives the colour 'papaya whip' to the cell, otherwise it gets the corresponding text and colour from the bg_colour and colour dictionaries.
    def paint_grid(self):
        for i in range(4):
            for j in range(4):
                if self.matrix_cell[i][j]==0:
